...
struct allocator {
	...
	void deallocate(T* p, size_t n) const noexcept {
		eastl::GetDefaultAllocator()->deallocate(p, n);
	}
...
#ifdef UNICODE
typedef tmpl::string<::ts_std::wstring> string;
typedef wchar_t char_t;
#define TXT(quote) L##quote
#define STR(quote) L##quote##_S
using tstring = ::ts_std::wstring;
using tostream = ::std::wostream;
using tostringstream = ::std::wostringstream;
using tstringstream = ::std::wstringstream;
#define to_tstring ::std::to_wstring
#else
typedef tmpl::string<::ts_std::string> string;
typedef char char_t;
#define TXT(quote) quote
#define STR(quote) quote##_S
using tstring = ::ts_std::string;
using tostream = ::std::ostream;
using tostringstream = ::ts_std::ostringstream;
using tstringstream = ::ts_std::stringstream;
...
template<class _Ty>
struct is_stringish : std::bool_constant<std::is_same_v<_Ty, const char_t*> || std::is_same_v<_Ty, ts_std::string> || std::is_same_v<_Ty, ts_std::wstring> || std::is_same_v<_Ty, std::string_view> || std::is_same_v<_Ty, std::wstring_view> || std::is_same_v<_Ty, string> || std::is_same_v<_Ty, any>> {
};
...
template<typename L, typename R = void>
	requires ArithmeticOrEnum<R> && (!ArithmeticOrEnum<L>)
...
struct string {
	...
	inline operator double() {
		return !(*this) ? 0 : std::stod(_value.c_str());
	}
	...
	string_t toUpperCase() {
		ts_std::string result(this->operator ts_std::string&());
		for (auto& c : result) {
			c = toupper(c);
		}
...
template<typename E>
struct array {
	using array_type_base = ts_std::vector<E>;
	...
	array(ts_std::vector<E> values) : _values(array_traits<array_type>::create(values)), isUndefined(false) {
	}
	...
	array slice(size_t first, size_t last) {
		return array(ts_std::vector<E>(get().cbegin() + first, get().cbegin() + last + 1));
	}
	...
	auto map(F p) -> array<undefined_t> {
		ts_std::vector<undefined_t> result;
	...
	/*
            ...
            {
                ts_std::vector<decltype(p(E()))> result;
                std::transform(get().begin(), get().end(), std::back_inserter(result), [=](auto &v)
                               { return mutable_(p)(v); });
            ...
            {
                ts_std::vector<decltype(p(E(), 0))> result;
                auto first = &(get())[0];
                std::transform(get().begin(), get().end(), std::back_inserter(result), [=](auto &v)
                               {
                                   auto index = &v - first;
...
*/
	...
	js::string join(js::string s) {
		return std::accumulate(get().begin(), get().end(), js::string{}, [&](auto& res, auto& piece) { return res += (res) ? s + piece : piece; });
	}
...
struct object {
	...
	using object_type_base = ts_std::unordered_map<K, V, K_hash, K_equal_to>;
	...
	any& operator[](ts_std::string s) const;
	...
	void Delete(js::number field) {
		get().erase(field.operator ts_std::string());
	}
...
struct any {
	...
	operator js::string() {
		/*
            // String
            template <typename T>
            string<T>::string(any val) : _value(val != null ? val.operator ts_std::string() : string_empty._value), _control(val != null ? string_defined : string_null)
            {
            }    
        */
	...
	operator std::function<Rx(Args...)>() const {
		if (get_type() == anyTypeId::function_type) {
			...
			return std::function<Rx(Args...)>([=](Args... args) -> Rx { return func->invoke({args...}); });
		}
...
namespace tmpl {
template<typename K, typename V>
inline void object<K, V>::Delete(js::any field) {
	get().erase(field.operator ts_std::string());
}
}// namespace tmpl
template<typename... Args>
...
any function_t<F, _MethodType>::invoke(std::initializer_list<any> args_) {
	auto args_vector = ts_std::vector<any>(args_);
...
// typedefs
typedef ts_std::unordered_map<any, int, any::any_hash, any::any_equal_to> switch_type;
...
string<T> string<T>::operator+(any value) {
	string tmp(_value);
	tmp._value.append(value.operator ts_std::string());
	return tmp;
}

template<typename T>
string<T>& string<T>::operator+=(any value) {
	auto value_string = value.operator ts_std::string();
	_control = string_defined;
...
any& object<K, V>::operator[](js::number n) const {
	return mutable_(get())[static_cast<ts_std::string>(n)];
}

template<typename K, typename V>
any& object<K, V>::operator[](js::number n) {
	return get()[static_cast<ts_std::string>(n)];
}

template<typename K, typename V>
any& object<K, V>::operator[](const char_t* s) const {
	return mutable_(get())[ts_std::string(s)];
}

template<typename K, typename V>
any& object<K, V>::operator[](ts_std::string s) const {
	return mutable_(get())[s];
}

template<typename K, typename V>
any& object<K, V>::operator[](js::string s) const {
	return mutable_(get())[s.operator ts_std::string&()];
}

template<typename K, typename V>
any& object<K, V>::operator[](const char_t* s) {
	return get()[ts_std::string(s)];
}

template<typename K, typename V>
any& object<K, V>::operator[](ts_std::string s) {
	return get()[s];
}

template<typename K, typename V>
any& object<K, V>::operator[](js::string s) {
	return get()[s.operator ts_std::string&()];
}
...
template<typename... Args>
object assign(const object& dst, const Args&... args) {
	for (auto src : {args...}) {
...
static number parseInt(const js::string& value, int base = 10) {
	return number(std::stol(value._value.c_str(), 0, base));
}

static number parseFloat(const js::string& value) {
	auto r = NaN;
	try {
		r = number(std::stod(value._value.c_str(), 0));
